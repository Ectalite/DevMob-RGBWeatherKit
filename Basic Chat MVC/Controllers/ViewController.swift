...
class ViewController: UIViewController {
    ...
    private var PixelXCharacteristic: CBCharacteristic!
    private var PixelYCharacteristic: CBCharacteristic!
...
extension ViewController: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?)
    {
        ...
        {
            print("Characteristic: \(characteristic.uuid) found")
            if characteristic.uuid.isEqual(CBUUIDs.BLE_Characteristic_uuid_PixelX)  {
              PixelXCharacteristic = characteristic
              BlePeripheral.connectedPixelXChar = PixelXCharacteristic
              //peripheral.readValue(for: characteristic)
              print("PixelX Characteristic: \(PixelXCharacteristic.uuid)")
            }

            if characteristic.uuid.isEqual(CBUUIDs.BLE_Characteristic_uuid_PixelY){
              PixelYCharacteristic = characteristic
              BlePeripheral.connectedPixelYChar = PixelYCharacteristic
              print("PixelY Characteristic: \(PixelYCharacteristic.uuid)")
            }

            if characteristic.uuid.isEqual(CBUUIDs.BLE_Characteristic_uuid_Color){
              ColorCharacteristic = characteristic
              BlePeripheral.connectedColorChar = ColorCharacteristic
              print("Color Characteristic: \(ColorCharacteristic.uuid)")
            }

            if characteristic.uuid.isEqual(CBUUIDs.BLE_Characteristic_uuid_Send){
              SendCharacteristic = characteristic
              BlePeripheral.connectedSendChar = SendCharacteristic
              print("Send Characteristic: \(SendCharacteristic.uuid)")
            }
        }
        delayedConnection()
    }
    ...
    print("Function line \(#line) was called ?")
...
extension ViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        print("IndexPath: \(peripheralArray.isEmpty)")
